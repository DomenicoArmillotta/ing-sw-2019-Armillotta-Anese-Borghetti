...
public class ReturnMoveOptionPushingWorkerTest {
    @Test
    public void ReturnTestWorkerPush() {
        ...
        Cell cellaDietro=match.getMap()[2][4];
        ...
        match.createGodList();
        worker11.getOwner().setPlayerGod(match.getGodList().get(God.MINOTAURO.ordinal()));
        //chiamo funzione
        List<Cell> MoveCells=new ArrayList<>();
        MoveCells=worker11.getOwner().getPlayerGod().getEffect().doReturnFirstMoveOptions(worker11);

        //NELLA CELLA21 ALTRO WORKER NON MIO E CELLADIETRO LIBERA QUINDI  AGGIUNGE
